...
|     `SnapshotOfLastIsolation`      | If the subhalo has ever been a satellite, the most recent output when this object was a central subhalo. Note that this is not always the same as the ouput before a subhalo became a satellite for the first time. If it has always been a central subhalo, it equals -1.    |
