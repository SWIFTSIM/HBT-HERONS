...
*   `LoadSubhalos`: load one or more properties of all subhaloes at a specific output number.
*   `GetTrackSnapshot`: load one or more properties of specific subhaloes at a specific output number.
*   `GetTrackEvolution`: load the evolution of one ore more properties of specific subhaloes. It returns the evolution from the time when they were first resolved until the last available HBT-HERONS output, even if the subhaloes have become an orphan.
