...
$$
K_{i} = \dfrac{1}{2}m_{i}|(\vec{v}_{i} - \vec{v}_{\rm com}) + H \times a(\vec{x}_{i} - \vec{x}_{\rm com})|^{2}.
$$

Here, $a$ is the expansion factor corresponding to the simulation output being analysed.
...
$$
U_{i} = \sum_{j}\dfrac{Gm_{i}m_{j}\theta_{\rm H}(- E_{\mathrm{bind},j})}{a|\vec{x}_{i} - \vec{x}_{j}|^{2}}.
$$
