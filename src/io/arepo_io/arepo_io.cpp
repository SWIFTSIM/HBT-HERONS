...
MPI_Datatype ArepoReader_t::AREPO_HEADER_MPI_datatype()
{
  ArepoHeader_t p;
#define NumAttr 13
...
#undef RegisterAttr
  ...
  MPI_Datatype MPI_AREPO_HEADER;
  MPI_Type_create_struct(i, blockcounts, offsets, oldtypes, &MPI_AREPO_HEADER);
  MPI_Type_create_resized(MPI_AREPO_HEADER, (MPI_Aint)0, extent, &MPI_AREPO_HEADER);
  MPI_Type_commit(&MPI_AREPO_HEADER);
#undef NumAttr

  return MPI_AREPO_HEADER;
}

/* To communicate vectors containing arrays of size TypeMax (e.g. halo length
 * per type, particles in each snapshot per type) */
MPI_Datatype ArepoReader_t::INT_ARRAY_MPI_datatype()
{
...
void ArepoReader_t::SetSnapshot(int snapshotId)
{
...
std::string ArepoReader_t::GetSnapshotFileName(int ifile)
{
  std::stringstream formatter;
...
std::string ArepoReader_t::GetGroupFileName(int ifile)
{
  std::string snap_idname = SnapshotName.substr(SnapshotName.size() - 3); // last 3 chars
  std::stringstream formatter;
   ...
   * depends on whether Arepoonly ran FoF or if it also did SUBFIND/SUBFIND-HBT. */
  if (HBTConfig.SnapshotDirBase.length() > 0)
  {
    /* If only FoF was run. */
...
ArepoHeader_t ArepoReader_t::ReadHeader(int ifile)
{
  ArepoHeader_t HeaderThisFile;

  std::string filename = GetSnapshotFileName(ifile);
  hid_t file = H5Fopen(filename.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);

  ReadAttribute(file, "Header", "NumFilesPerSnapshot", H5T_NATIVE_INT, &HeaderThisFile.NumberOfFiles);
  ReadAttribute(file, "Header", "BoxSize", H5T_NATIVE_DOUBLE, &HeaderThisFile.BoxSize);
  ReadAttribute(file, "Header", "Time", H5T_NATIVE_DOUBLE, &HeaderThisFile.ScaleFactor);
  ReadAttribute(file, "Parameters", "Omega0", H5T_NATIVE_DOUBLE, &HeaderThisFile.OmegaM0);
  ReadAttribute(file, "Parameters", "OmegaLambda", H5T_NATIVE_DOUBLE, &HeaderThisFile.OmegaLambda0);

  /* Per particle type information. */
  for (int i = 0; i < TypeMax; i++)
  {
    HeaderThisFile.mass[i] = 0.;
    HeaderThisFile.npart[i] = 0;
    HeaderThisFile.npartTotal[i] = 0;
  }
  ReadAttribute(file, "Header", "MassTable", H5T_NATIVE_DOUBLE, HeaderThisFile.mass);
  ReadAttribute(file, "Header", "NumPart_ThisFile", H5T_NATIVE_INT, HeaderThisFile.npart);
  ReadAttribute(file, "Header", "NumPart_Total", H5T_HBTInt, HeaderThisFile.npartTotal);

  /* Read unit system used by Arepo */
  double length_cgs;
  ...
  HeaderThisFile.MassInMsunh  = mass_cgs / 1.98841e33;
  HeaderThisFile.LengthInMpch = length_cgs / 3.08567758e24;
  HeaderThisFile.VelInKmS = velocity_cgs / 1e5;
  ...
  return HeaderThisFile;
}

int ArepoReader_t::ReadGroupFileCounts(int ifile)
{
  int nfiles;
  std::string filename = GetGroupFileName(ifile);
  hid_t file = H5Fopen(filename.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
...
HBTInt ArepoReader_t::ReadGroupFileTotParticles(int ifile)
...
void ArepoReader_t::GetParticleCountInFile(hid_t file, HBTInt np[])
{
...
void ArepoReader_t::CompileSnapshotParticleOffsets(int NumberFiles)
{
...
HBTInt ArepoReader_t::CompileGroupFileOffsets(std::vector<HBTInt> &nhalo_per_groupfile,
 ...
 * halo lengths provided in the Arepo group catalogues. */
void ArepoReader_t::ReadSnapshot(int FirstFileIndex, int CurrentFileIndex, Particle_t *ParticlesInFile)
{
  /* Open the simulation output to read */
  std::string filename = GetSnapshotFileName(CurrentFileIndex);
 ...
 * group file. */
void ArepoReader_t::ReadGroupLen(int ifile, HBTInt *buf)
{
  /* Open the group file. */
  std::string filename = GetGroupFileName(ifile);
  hid_t file = H5Fopen(filename.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
 ...
 * group file. */
void ArepoReader_t::ReadGroupLenPerType(int ifile, std::array<HBTInt, TypeMax> *buf)
{
  /* Open the group file. */
  std::string filename = GetGroupFileName(ifile);
  hid_t file = H5Fopen(filename.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
...
void ArepoReader_t::LoadSnapshot(MpiWorker_t &world, int snapshotId, std::vector<Particle_t> &Particles,
                                   Cosmology_t &Cosmology)
{
  ...
  MPI_Bcast(&Header, 1, MPI_AREPO_HEADER, root_node, world.Communicator);
  ...
  /* Read host halo of each particle. Arepo has particle and group information
 ...
 * rank. */
void ArepoReader_t::LoadHaloSizes(MpiWorker_t &world)
{
...
void ArepoReader_t::GetNumberParticlesPerRank(MpiWorker_t &world, const std::vector<Particle_t> &Particles)
{
  if (world.rank() == root_node)
  {
...
struct HaloPartitioner_t
{
  ...
  void Fill(const std::vector<HBTInt> &HaloSizes, const std::vector<HBTInt> &ProcLen)
  {
     ...
     * halo segment is. We make use of the fact that Arepo groups are sorted
...
void ArepoReader_t::LoadParticleProperties(MpiWorker_t &world, std::vector<Particle_t> &Particles)
{
...
void ArepoReader_t::LoadParticleHosts(MpiWorker_t &world, std::vector<Particle_t> &Particles)
{
...
void ArepoReader_t::CreateHaloSegments(const std::vector<Particle_t> &SnapshotParticles, std::vector<Halo_t> &Halos)
{
...
/* Creates and populates the Halos of the current snapshot, which for Arepo is
 * done using the snapshot particles. */
void ArepoReader_t::LoadGroups(MpiWorker_t &world, const ParticleSnapshot_t &partsnap, std::vector<Halo_t> &Halos)
{
  ...
  /* NOTE: by communicating Arepo particles before creating halo segments, I
   * have indirectly created many more halo segments than what would be required
   * if I had not communicated them. This is because particles in Arepo
...
bool IsArepoGroup(const std::string &GroupFileFormat)
{
  return GroupFileFormat.substr(0, 5) == "arepo";
}
} // namespace ArepoReader
